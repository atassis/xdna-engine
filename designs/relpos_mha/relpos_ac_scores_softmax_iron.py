...
# (designs/relpos_mha/relpos_mha.cc): given packed qk[2T,DK] bf16
...
def my_relpos_ac_scores_softmax(dev, T):
    ...
    def sequence(QK, BD, PR, qk_h, bd_h, probs_h):
        qk_h.fill(QK)
        bd_h.fill(BD)
    ...
    rt = Runtime(
        sequence,
        [qk_ty, bd_ty, probs_ty, of_qk.prod(), of_bd.prod(), of_probs.cons()],
    )
