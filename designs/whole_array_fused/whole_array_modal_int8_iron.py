...
def _str2bool(v):
    ...
    # selected the r=8 bfp16 tile layout while design_override.mk's make-level `ifeq (...,1)`
...
def my_matmul(
    ...
    # MODAL int8 design (L3): i8 in / i32 accumulate / i32 C TILE; the epilogue dequants
    # i32 -> f32 IN-PLACE (×S from RTP), so the host epilogue becomes a near-no-op like the
    # bf16 modal. Same in-place-reduce structure as whole_array_modal_iron.py (no separate
    # acc_buf — the matmul reduces into the C tile), which is why it fits L1 just like f32-out.
    # NO K-augmentation here: int8 bias/SiLU stay on host, so K is the real K (768), not 800.
    dtype_in_str = "i8"
    dtype_acc_str = "i32"
    dtype_out_str = "i32"
    ...
    # r, s, t are the dimensions required by the microkernel MAC instructions (i8: 8x8x8).
    r, s, t = microkernel_mac_dim_map[dev]["i8"]
    ...
    # AIE Core Function declarations. zero/matmul operate on the i32 accumulator (cacc_ty,
    # == the i32 C tile — in-place reduce); the epilogue reads i32 acc, dequants to f32 in-place.
    zero_kernel = Kernel(f"zero_{dtype_acc_str}", f"mm_{m}x{k}x{n}.o", [cacc_ty])
    matmul_kernel = Kernel(
        f"matmul_{dtype_in_str}_{dtype_acc_str}",  # matmul_i8_i32
        f"mm_{m}x{k}x{n}.o",
        [A_l1_ty, B_l1_ty, cacc_ty],
    )
    # MODAL int8 dequant epilogue: rtp[0] carries the per-dispatch dequant scalar S as f32 bits
    # (host patches scale_a*w_scale into the inst-stream RTP before each dispatch). Single mode
    # (dequant); bias/SiLU stay on host. The C++ writes f32 into the i32 4B C tile in-place, so
    # the epilogue's 2nd arg is declared C_l1_ty (i32, 4B) and reinterpreted as float* in-kernel.
    rtp_ty = np.ndarray[(16,), np.dtype[np.int32]]
    epilogue_kernel = Kernel(
        "mm_modal_dequant_i32_f32",
    ...
    # Tasks for each worker to perform. The i32 C tile == the i32 accumulator, so the matmul reduces
    # DIRECTLY into the C output tile and the epilogue dequants IN-PLACE (i32->f32, both 4B) — no
    # separate acc_buf, exactly like the f32-out modal. That single-4B-tile footprint is what lets the
    # wide int8 tile (64x64x96) fit L1 (the bf16-OUT dual-buffer overflow, 07b, does not apply here).
    def core_fn(in_a, in_b, out_c, rtp_buff, barrier, zero, matmul, epilogue):
        barrier.wait_for_value(1)  # wait for the host to write the dequant scalar S into rtp[0]
        ...
        for _ in loop:
            ...
            for _ in range_(K // k):
                ...
                in_a.release(1)
                in_b.release(1)
            epilogue(elem_out, elem_out, rtp_buff)  # in-place dequant i32->f32; rtp[0]=S (f32 bits)
            out_c.release(1)

    # Per-core RTP (dequant scalar S) + a shared runtime barrier. Each worker reads rtp[0] as f32 bits.
    ...
    def sequence(A, B, C, A_prods, B_prods, C_conses):
        nonlocal c_index
        # bake a PLACEHOLDER dequant scalar S into this stream's RTP (rtp[0] = f32 bits of S), then
        # release the barrier so the cores read it. 0x3F800000 == 1.0f: a no-op scale, correct for the
        # build/smoke test (compile + L1-fit + identity-dequant numerics). At runtime the host patches
        # the real per-dispatch S = scale_a * w_scale into this RTP slot in the instruction stream
        # before each dispatch (the per-op scale delivery; do_silu is unused for the int8 dequant path).
        s_bits = 0x3F800000  # bitcast(1.0f)
...
def main():
    ...
    # This design is fixed to i8 in / i32 accumulate / i32 C tile (epilogue dequants to f32).
    argparser.add_argument("--dtype_in", type=str, default="i8", choices=["i8"])
    argparser.add_argument("--dtype_out", type=str, default="i32", choices=["i32"])
