...
def my_matmul(
    ...
    # This fused design is bf16 in / f32 accumulate / bf16 out only.
    dtype_in_str = "bf16"
    dtype_acc_str = "f32"
    dtype_out_str = "bf16"
    ...
    epi_fn_name = (
        "mm_silu_epilogue_f32_bf16" if do_silu else "mm_narrow_epilogue_f32_bf16"
    )
    ...
    # acc is a core-local f32 buffer; out_c carries the bf16 output tile.
    def core_fn(in_a, in_b, out_c, acc, zero, matmul, epilogue):
        ...
        for _ in loop:
            ...
            for _ in range_(K // k):
                ...
                in_a.release(1)
                in_b.release(1)
            epilogue(acc, elem_out)
            out_c.release(1)

    # Set up compute tiles. Each worker gets its own f32 accumulator buffer.
    workers = []
