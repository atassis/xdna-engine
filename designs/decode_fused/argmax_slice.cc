...
// per-file (see argmax_op.py's SourceArtifact), same as every other kernel here, and the
...
// designs/decode_fused, which has no verified include path to that header (unlike
// aie_kernels/*.cc). Semantically inert either way — they only shape scheduling.
