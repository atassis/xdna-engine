...
#
# Device-free. Run:  bash scripts/lnaffcast_scatter_build.sh
set -u
...
build_arm() {
  local ctg="$1" sfx rtp
  rtp="${RTP:-1}"
  if [ "$rtp" = 1 ]; then sfx="${BASE}lnaff1024scatrtp18g4${ctg:+ctg$ctg}"
  else                    sfx="${BASE}lnaff1024scat"; fi
  log "\n[build] $sfx"
  ( cd "$EX" && WA_C_DEPTH=1 WA_AB_DEPTH=2 WA_L3L2_DEPTH=2 \
      ...
      lnaffcast_scatter_c=1 lnaffcast_contig_taps="$ctg" \
...
a="$B/aie_${BASE}lnaff1024scatrtp18g4.mlir"
b="$B/aie_${BASE}lnaff1024scatrtp18g4ctgc.mlir"
