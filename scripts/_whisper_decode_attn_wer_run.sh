#!/usr/bin/env bash
# Single-tenant Whisper-small WER head-to-head: NPU decode step-1 (host self-attn) vs NPU decode
# with on-chip SELF-attention (NPU_DECODE_ATTN: fused LN+QKV + mha_decode kernel), over wer_clips.
# Stops the NPU services, runs BOTH evals (each launches engine_serve, POSTs the clips, scores WER
# vs refs.json), then ALWAYS restarts the services (trap, even on failure/Ctrl-C).
#
...
run_one step1    "NPU_DECODE=1"
run_one npu_attn "NPU_DECODE=1 NPU_DECODE_ATTN=1"
