#!/usr/bin/env python3
"""On-device validation of the STEP-2 COMPOSED rel-pos block: on-chip AC matmul
feeding the resident-in-L1 scores->softmax brick.
...
The kernel bakes inv_scale = 1/sqrt(DK). Block-0's raw scores saturate (one-hot
softmax); in that regime the bf16 matmul can flip a near-tie argmax (a harmless
sqrt(2/T) jump that washes out end-to-end -- see golden G5a/G3). So by DEFAULT the
host pre-scales qu/BD by 1/std to land a NON-DEGENERATE softmax that exercises the
on-chip matmul + exp2 path (the oracle uses the identical effective scale). Pass
--raw to drive the true saturating regime instead.

Gate: rel-L2 <= 0.08 AND corr >= 0.99 vs the fp32 host softmax.

IRON host ABI (dwconv1d/eltwise_mul test.cpp): opcode=3;
  kernel(opcode, instr[gid1,cacheable], n_instr, QK[gid3], BD[gid4], PROBS[gid5])
QK = [2*T*DK] bf16, BD = [T*P] f32, PROBS = [T*T] bf16.
"""
...
def main():
    ...
    ap.add_argument("--raw", action="store_true",
                    help="drive the real saturating regime (default: rescale to a "
                         "non-degenerate softmax that exercises the matmul + exp2)")
    a = ap.parse_args()
    ...
    ac_f32 = (qu @ kh.T).astype(np.float32)                    # [T,T] f32 reference AC
    bd = (qv @ pm[:, h].T).astype(np.float32)                  # [T,P]

    # AC is computed ON DEVICE from qu,k (bf16 mmul). To hit a non-degenerate
    # softmax we pre-divide qu and BD by std(scores) host-side (AC = qu@k^T is
    # bilinear, so scaling qu scales AC by the same factor); the oracle uses the
    # identical effective scale. --raw drives the true saturating (one-hot) regime.
    if a.raw:
        qu_dev, k_dev, bd_dev, oracle_scale = qu, kh, bd, inv_scale
    else:
        bd_sh = rel_shift_host(bd[None])[0]
        std = float(((ac_f32 + bd_sh) * inv_scale).std()) + 1e-6
        qu_dev, k_dev, bd_dev, oracle_scale = qu / std, kh, bd / std, inv_scale / std

    ref = host_probs(ac_f32, bd, oracle_scale)                 # fp32 golden [T,T]

    # Pack qu then k contiguously as bf16 -> qk[2*T*DK].
    QK = np.concatenate([
        np.ascontiguousarray(qu_dev, dtype=np.float32).astype(bfloat16).reshape(-1),
        np.ascontiguousarray(k_dev, dtype=np.float32).astype(bfloat16).reshape(-1),
    ])
    ...
    def once():
        r = kk(3, bo_instr, instr.size, bo_qk, bo_bd, bo_pr)
        r.wait()
    ...
    rowsum = yf.sum(-1)
    print(f"[run] device time/iter: {dt*1e3:.3f} ms  (T={T} x T={T}; on-chip AC matmul + resident softmax)")
    ...
    print(f"[run] relpos_ac_scores_softmax on NPU: {'PASS' if ok else 'FAIL'}")
