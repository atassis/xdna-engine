#!/usr/bin/env bash
# Bump our mlir-aie fork integration branch (atassis/mlir-aie:integration-stack) onto the latest upstream:
...
#   scripts/bump_upstream.sh --apply         # do it for real: rebase integration-stack -> rebuild instance + CPU
...
git -C "$SUB" cat-file -e "${MLIR_AIE_FORK_COMMIT}^{commit}" 2>/dev/null || git -C "$SUB" fetch -q fork integration-stack
...
# Rebase in a THROWAWAY worktree so integration-stack (and the live submodule checkout) are untouched until --apply.
...
if git -C "$WT" -c rerere.enabled=false rebase --onto "$NEWUP" "$BASE" 2>/tmp/bump-rebase.err; then
  ...
  if [ "$APPLY" -eq 1 ]; then
    echo "[bump] --apply: moving integration-stack to the rebased tip + re-pinning toolchain.lock ..." >&2
    git -C "$SUB" branch -f integration-stack "$NEWTIP"
    ...
    echo "[bump] re-pinned. NEXT: scripts/toolchain_smoke.sh (CPU gate) -> if green, device-revalidate + push integration-stack + delete the dropped per-PR branches. If the smoke FAILS, revert toolchain.lock."
  else
    echo "[bump] DRY-RUN: no changes made. Re-run with --apply to move integration-stack + re-pin (then smoke + revalidate)."
  fi
else
  ...
  echo "[bump] => that patch needs a manual rebase (\"rebase this PR\"). Resolve in a real worktree of integration-stack."
