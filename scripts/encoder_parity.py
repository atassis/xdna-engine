...
Usage:
  ...
  cargo run -p npu-dev --release -- parakeet-encode artifacts/wer_mels /tmp/enc_ref --cpu
  NPU_XCLBIN_ROOT=$PWD cargo run ... -p npu-dev -- parakeet-encode artifacts/wer_mels /tmp/enc_ship
  PARAKEET_FUSED_BLOCK=1 NPU_XCLBIN_ROOT=$PWD cargo run ... -- parakeet-encode artifacts/wer_mels /tmp/enc_cand
  # 2. gate:
...
def main():
    ...
    ap.add_argument("--max-new-burst", type=float, default=0.80,
                    help="absolute cap on how much per-frame rel-err any single frame may GAIN. "
                         "The device is bit-deterministic, so a same-config rerun measures exactly "
                         "0.0 here and any value above 0 is real. "
                         "PRICED 2026-08-03 (was 0.15, which only matched --burst-floor and was "
                         "self-referential). burst_sensitivity.py injects a len-3 burst of known "
                         "magnitude into the DEVICE baseline and decodes, n=5712: P(transcript "
                         "changes) = 0.000/0.004/0.007/0.007/0.015/0.092/0.342 at magnitude "
                         "0.05/0.15/0.24/0.42/0.80/1.20/1.60, against a 0.004 noise floor (the "
                         "0.05 negative control scores the same as 0.42). 0.80 is the largest "
                         "magnitude still within ~4x that floor and sits well below the 1.20 "
                         "turn-up. It also has to clear 0.4229, the new-burst of the fc1 "
                         "pack-in-drain fold, which SHIPS by default -- a gate the shipped "
                         "configuration fails is broken. Sweep granularity is coarse between 0.42 "
                         "and 0.80; refine there before trusting the third digit. "
                         "THIS CAP IS TIED TO THE 17-CLIP CORPUS and does not transfer. The "
                         "statistic is an unnormalized MAX over frames, so it grows with corpus "
                         "size at fixed arm quality: measured on the SAME fused arm, median max = "
                         "0.2909 / 0.4375 / 0.7522 / 0.8492 at 17 / 50 / 100 / 200 clips. 7.7%% of "
                         "17-clip draws exceed 0.80 where the full 200-clip set always does. So "
                         "running MORE evaluation makes a good arm fail, which is backwards, and "
                         "any cap is implicitly a cap on (quality x n_clips). Re-derive it if you "
                         "change the corpus. The real fix is to stop summarising with a max -- a "
                         "high quantile of the per-frame gain, or a count of frames over a "
                         "threshold divided by frame count, is scale-stable; that is a redesign, "
                         "not a re-pricing, and has not been done. "
                         "KNOW ITS LIMIT: at clip granularity this statistic does not rank the "
                         "outcome. Over 200 FLEURS clips and four device arms its ROC AUC against "
                         "'did the transcript change' is 0.492-0.510 (0.5 = chance), and three "
                         "arms have now FAILED it at no measured transcript cost -- conveyor "
                         "(transcript-BETTER), the fused block (176 vs 176 word edits, exactly "
                         "equal), the fold (2 edits in 3613, sign flips with the reference). So a "
                         "PASS here is not evidence of safety and a FAIL is weak evidence of harm; "
                         "this cap bounds a worst case, it does not predict one. To decide a ship, "
                         "decode both arms and diff the transcripts.")
