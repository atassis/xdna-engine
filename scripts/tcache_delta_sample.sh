...
N="${1:-5}"
# Optional device serializer. The NPU is single-tenant, so two timed runs overlapping produce
# numbers that mean nothing -- but the lock helper is not part of this repo. Point NPU_LOCK at one
# (it is invoked as `$NPU_LOCK run -- <cmd>`); with it unset the run proceeds and the CALLER owns
# serialization. Unset is a real mode, not a fallback: `fuser /dev/accel/accel0` before starting.
LOCK="${NPU_LOCK:-}"
...
run_one(){ # $1 tag  $2 base dir  $3 tr dir
  ...
  if [ -n "$LOCK" ]; then
    ...
    bash scripts/tcache_e2e_parity.sh "$WT/artifacts/$2" "$WT/artifacts/$3" 256 \
      > "$OUT/$1.log" 2>&1
  fi
