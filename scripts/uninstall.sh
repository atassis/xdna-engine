...
# legacy parakeet_serve / npu-asr service has been retired, so there is nothing to revert to - reinstall with
# scripts/install.sh.
set -euo pipefail
...
rm -f "$BIN_DIR/npu"
echo "    (kept: $CFG_DIR/engine.toml, ~/.local/lib/xdna-engine, artifacts/. remove manually for a full wipe.)"
