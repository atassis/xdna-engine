...
```
cargo run -p npu-dev -- conveyor-parity      # gate rel-L2 <= 5e-3
```
...
NPU_XCLBIN_ROOT=$PWD \
  cargo run -p npu-dev --release -- parakeet-encode artifacts/wer_mels artifacts/wer_enc_shipped
...
PARAKEET_CONVEYOR_MHA=1 NPU_XCLBIN_ROOT=$PWD \
  cargo run -p npu-dev --release -- parakeet-encode artifacts/wer_mels artifacts/wer_enc_conveyor
...
PARAKEET_PHASE_TIMING=1 NPU_XCLBIN_ROOT=$PWD \
  cargo run -p npu-dev --release -- parakeet-encode artifacts/wer_mels /tmp/enc_shipped
PARAKEET_PHASE_TIMING=1 PARAKEET_CONVEYOR_MHA=1 NPU_XCLBIN_ROOT=$PWD \
  cargo run -p npu-dev --release -- parakeet-encode artifacts/wer_mels /tmp/enc_conveyor
