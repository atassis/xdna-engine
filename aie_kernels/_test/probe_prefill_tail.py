...
TAIL = int(os.environ.get("TAIL_PRESENT", "0"))
    ...
    try:
        res = bricklib.verify_streamed(
            ...
            compile_flags=[f"-DPREFILL_HD={HD}", f"-DPREFILL_M={M}",
                           f"-DPREFILL_STOP_AFTER={stop}", f"-DPREFILL_VEC_EMIT={VEC}", f"-DPREFILL_SM_OUT_DIRECT={DIRECT}", f"-DPREFILL_SM_IN_STREAMED={INSTR}", f"-DPREFILL_SET_ROUNDING={ROUND}", f"-DPREFILL_SM_WARM={WARM}", f"-DPREFILL_TAIL_PRESENT={TAIL}"],
        )
