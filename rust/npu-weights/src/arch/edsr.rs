...
// (the im2col->GEMM frontier flattens per-layer). Weights -> bf16, biases -> f32. Flat checkpoint names match
...
/// Map a torch state_dict key to the flat checkpoint name, or None to skip.
fn checkpoint_name(key: &str) -> Option<String> {
...
impl Arch for Edsr {
    ...
    fn transform(
        ...
        for (k, t) in src {
            let Some(name) = checkpoint_name(k) else { continue };
...
mod tests {
    ...
    #[test]
    fn maps_edsr_keys() {
        assert_eq!(checkpoint_name("head.0.weight").as_deref(), Some("head_w"));
        assert_eq!(checkpoint_name("body.3.body.0.weight").as_deref(), Some("b3_c0_w"));
        assert_eq!(checkpoint_name("body.7.body.2.bias").as_deref(), Some("b7_c1_b"));
        assert_eq!(checkpoint_name("body.16.weight").as_deref(), Some("btail_w"));
        assert_eq!(checkpoint_name("tail.0.0.weight").as_deref(), Some("tail0_w"));
        assert_eq!(checkpoint_name("tail.1.bias").as_deref(), Some("tail1_b"));
        assert_eq!(checkpoint_name("sub_mean.weight").as_deref(), Some("sub_mean_w"));
        assert_eq!(checkpoint_name("add_mean.bias").as_deref(), Some("add_mean_b"));
    }
