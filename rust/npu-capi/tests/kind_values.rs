...
//! 4 = tts. A renumber silently breaks every compiled consumer, so it is pinned by a test rather
//! than by a comment. The mapping is deliberately restated here rather than imported: that is what
...
fn kind_discriminants_are_append_only() {
    ...
    assert_eq!(code(ModelKind::Tts), 4);
}
