...
impl ConformerEncoder {
    pub fn new(dev: Rc<Device>, root: &Path, weights_dir: &Path) -> Result<Self, EngineError> {
        let ws = WeightStore::load(weights_dir)
            .map_err(|e| EngineError::Load(format!("encoder weights: {e}")))?;
        let n = ws.nblocks();
    ...
    pub fn build(cfg: &ScenarioConfig, root: &Path, dev: Rc<Device>) -> Result<Self, EngineError> {
        ...
        let load = |f: &str| -> Result<Session, EngineError> {
            Session::load(&env, asr.join(f).to_str().unwrap())
                .map_err(|e| EngineError::Load(format!("onnx load {f}: {e}")))
        };
        ...
        Ok(AsrPipeline { prep, decoder, joint, enc, vocab, _env: env })
    }

    fn decode(&self, encoded: &Array2<f32>, valid: usize) -> Result<Vec<i64>, EngineError> {
...
impl AsrModel for AsrPipeline {
    fn transcribe(&self, samples: &[i16]) -> Result<String, EngineError> {
        ...
        let feat = self.prep.run(
            ...
            &["features", "features_lens"]).map_err(|e| EngineError::Device(format!("preproc: {e}")))?;
