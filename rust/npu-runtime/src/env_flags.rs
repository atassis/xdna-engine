...
pub const FLAGS: &[Flag] = &[
    ...
    Flag { name: "NPU_LLM_REUSE_KV", owner: "npu-engine", site: "npu-engine/src/llm/npu_decode.rs",
    ...
    Flag { name: "NPU_PAR_SUBSAMPLE", owner: "npu-asr-host", site: "npu-asr-host/src/lib.rs",
        semantics: NotZero, default: "true",
        doc: "host-side subsample matmul runs multithreaded via rayon; opt out with =0." },
    Flag { name: "NPU_HOST_PROF", owner: "npu-asr-host", site: "npu-asr-host/src/prof.rs",
    ...
    Flag { name: "NPU_GLU_FUSED", owner: "npu-asr", site: "npu-asr/src/tuning.rs",
        semantics: NotZero, default: "true",
    ...
    Flag { name: "NPU_MM2_PIPELINE", owner: "npu-asr", site: "npu-asr/src/tuning.rs",
        semantics: NotZero, default: "true",
        doc: "pipelines the FFN's second matmul (mm2)." },
    Flag { name: "NPU_INT8_FASTEPI", owner: "npu-asr", site: "npu-asr/src/tuning.rs",
        semantics: NotZero, default: "true",
        doc: "fast int8 epilogue path; only meaningful under int8 precision." },
    Flag { name: "NPU_LN_NPU", owner: "npu-asr", site: "npu-asr/src/tuning.rs",
        semantics: IsOne, default: "false",
        doc: "LayerNorm dispatches on the NPU instead of host." },
    Flag { name: "NPU_QKV_OVERLAP", owner: "npu-asr", site: "npu-asr/src/tuning.rs",
        semantics: IsOne, default: "false",
    ...
    Flag { name: "NPU_ENC_FFN_RESIDENT", owner: "npu-asr", site: "npu-asr/src/tuning.rs",
        semantics: IsOne, default: "false",
    ...
    Flag { name: "NPU_CONV_TRANSPOSE", owner: "npu-asr", site: "npu-asr/src/conv_npu.rs",
    ...
    Flag { name: "NPU_KERNEL_MANIFEST_VERIFY", owner: "npu-asr, npu-engine, npu-parakeet",
        site: "npu-asr/src/conv_npu.rs", semantics: IsOk, default: "false",
    ...
    Flag { name: "NPU_TILE", owner: "npu-asr", site: "npu-asr/src/ctx2.rs",
    ...
    Flag { name: "NPU_QUIET", owner: "npu-cli", site: "npu-cli/src/main.rs",
    ...
    // -- npu-engine -------------------------------------------------------------------------------
    Flag { name: "NPU_XCLBIN_ROOT", owner: "npu-engine", site: "npu-engine/src/asr/parakeet.rs",
    ...
    Flag { name: "NPU_DECODE_ATTN", owner: "npu-engine", site: "npu-engine/src/asr/whisper_decoder.rs",
        semantics: IsOk, default: "false",
    ...
    Flag { name: "NPU_DECODE_FUSED_PATCH", owner: "npu-engine", site: "npu-engine/src/asr/whisper_decoder.rs",
        semantics: IsOk, default: "false",
        doc: "opts OUT of the resident scratchpad-arena decode path back to legacy per-token ELF \
              patch/reload (A/B); set = legacy path." },
    Flag { name: "FUSED_PHASE_TIMING", owner: "npu-engine", site: "npu-engine/src/asr/whisper_decoder.rs",
        semantics: IsOk, default: "false",
        doc: "per-phase timing breakdown for the fused decode path. Read again at line 1817." },
    Flag { name: "NPU_DECODE_FUSED_REUSECTX", owner: "npu-engine", site: "npu-engine/src/asr/whisper_decoder.rs",
        semantics: IsOk, default: "false",
        doc: "persistent hw_context across decode tokens (no per-token re-registration)." },
    Flag { name: "NPU_DECODE_FUSED_HOSTCROSS", owner: "npu-engine", site: "npu-engine/src/asr/whisper_decoder.rs",
        semantics: IsOk, default: "false",
        doc: "opts OUT of the on-device cross-K/V GEMM fold back to the host f32 fold (A/B); set \
              = host fold. Read again at line 1820." },
    Flag { name: "NPU_DECODE_PROJOUT_CTX2", owner: "npu-engine", site: "npu-engine/src/asr/whisper_decoder.rs",
        semantics: IsOk, default: "false",
    ...
    Flag { name: "NPU_DECODE_PROJOUT_ELF", owner: "npu-engine", site: "npu-engine/src/asr/whisper_decoder.rs",
    ...
    Flag { name: "NPU_DECODE_PROJOUT_ELF_DIR", owner: "npu-engine", site: "npu-engine/src/asr/whisper_decoder.rs",
        semantics: Value, default: "<fused_decode_dir>/../projout_elf",
        doc: "overrides the proj_out ELF artifact directory." },
    Flag { name: "INT8_CK_HEADROOM", owner: "npu-engine", site: "npu-engine/src/asr/whisper_decoder.rs",
        semantics: Value, default: "1.0",
        doc: "int8 quantization headroom multiplier for cross-attn K (per-utterance per-channel scale)." },
    Flag { name: "INT8_CV_HEADROOM", owner: "npu-engine", site: "npu-engine/src/asr/whisper_decoder.rs",
        semantics: Value, default: "1.0",
        doc: "int8 quantization headroom multiplier for cross-attn V." },
    Flag { name: "WHISPER_ENC_HOST", owner: "npu-engine", site: "npu-engine/src/asr/whisper.rs",
        semantics: IsOk, default: "false",
        doc: "runs the Whisper encoder on host ONNX instead of the NPU; opt-in, loud (never a \
              silent fallback)." },
    Flag { name: "NPU_DECODE_FUSED", owner: "npu-engine", site: "npu-engine/src/config.rs",
        semantics: IsOk, default: "false",
    ...
    Flag { name: "NPU_DECODE", owner: "npu-engine", site: "npu-engine/src/config.rs",
        semantics: IsOk, default: "false",
    ...
    Flag { name: "NPU_DECODE_FUSED_BATCH", owner: "npu-engine", site: "npu-engine/src/asr/whisper.rs",
        semantics: IsOk, default: "false",
        doc: "batched/offline-bulk fused decoder subsystem, independent of the single-stream backend." },
    Flag { name: "NPU_DECODE_FUSED_BATCH_DIR", owner: "npu-engine", site: "npu-engine/src/asr/whisper.rs",
    ...
    Flag { name: "NPU_DECODE_FUSED_DIR", owner: "npu-engine", site: "npu-engine/src/asr/whisper.rs",
    ...
    Flag { name: "WHISPER_LANG_PIN", owner: "npu-engine", site: "npu-engine/src/asr/whisper.rs",
        semantics: IsOk, default: "false",
    ...
    Flag { name: "WHISPER_TIMING", owner: "npu-engine", site: "npu-engine/src/asr/whisper.rs",
        semantics: IsOk, default: "false",
        doc: "per-stage (prep/encode/decode) wall-clock timing for one transcribe window." },
    Flag { name: "NPU_DEBUG_TOKEN_IDS", owner: "npu-engine", site: "npu-engine/src/asr/whisper.rs",
        semantics: IsOk, default: "false",
    ...
    Flag { name: "PARAKEET_SUBSAMPLE_OUT_NPU", owner: "npu-parakeet", site: "npu-parakeet/src/encoder.rs",
        semantics: NotZero, default: "false",
        doc: "subsampling output stage dispatches on NPU." },
    Flag { name: "PARAKEET_MHA_HOSTQKV", owner: "npu-parakeet", site: "npu-parakeet/src/encoder.rs",
    ...
    Flag { name: "PARAKEET_MHA_QKV_AB", owner: "npu-parakeet", site: "npu-parakeet/src/encoder.rs",
        semantics: IsOk, default: "false",
        doc: "A/B diagnostic: resident LN->QKV vs host layernorm+matmul, rel-L2 per projection." },
    Flag { name: "PARAKEET_MHA_AB", owner: "npu-parakeet", site: "npu-parakeet/src/encoder.rs",
        semantics: IsOk, default: "false",
        doc: "A/B diagnostic: resident MHA context vs f32 host golden, for head 0 and a mid head." },
    Flag { name: "PARAKEET_CONVEYOR_MHA", owner: "npu-parakeet", site: "npu-parakeet/src/encoder.rs",
        semantics: IsOk, default: "false",
    ...
    Flag { name: "PARAKEET_CONVEYOR_BD", owner: "npu-parakeet", site: "npu-parakeet/src/npu.rs",
    ...
    Flag { name: "PARAKEET_RELPOS_ROWTILE", owner: "npu-parakeet", site: "npu-parakeet/src/npu.rs",
        semantics: Presence, default: "false",
        doc: "selects the row-tiled relpos bucket table (must match the artifact set PARAKEET_RELPOS_DIR names)." },
    Flag { name: "PARAKEET_RELPOS_NOPEEL", owner: "npu-parakeet", site: "npu-parakeet/src/npu.rs",
        semantics: Presence, default: "false",
        doc: "selects the no-peel relpos bucket table; only checked when ROWTILE is unset." },
    Flag { name: "PARAKEET_LN_MODE", owner: "npu-parakeet", site: "npu-parakeet/src/npu.rs",
        semantics: NotZero, default: "false",
    ...
    Flag { name: "PARAKEET_RELPOS_DIR", owner: "npu-parakeet", site: "npu-parakeet/src/npu.rs",
    ...
    Flag { name: "PARAKEET_RELPOS_NO_BUCKET", owner: "npu-parakeet", site: "npu-parakeet/src/npu.rs",
        semantics: Presence, default: "false",
        doc: "forces the ceiling relpos bucket for every clip (pre-T-bucketing baseline, A/B)." },
    Flag { name: "PARAKEET_RELPOS_2BUCKET", owner: "npu-parakeet", site: "npu-parakeet/src/npu.rs",
        semantics: Presence, default: "false",
    ...
    Flag { name: "PARAKEET_RELPOS_TACTIVE_PROBE", owner: "npu-parakeet", site: "npu-parakeet/src/npu.rs",
        semantics: Value, default: "unset",
    ...
    Flag { name: "ENC_PEROP_TIMING", owner: "npu-whisper", site: "npu-whisper/src/encoder.rs",
        semantics: IsOk, default: "false",
        doc: "per-op timing breakdown for the Whisper encoder forward pass." },

    // -- npu-xrt ----------------------------------------------------------------------------------
    Flag { name: "NPU_QUIET", owner: "npu-xrt", site: "npu-xrt/src/lib.rs",
        semantics: NotZero, default: "false",
    ...
    Flag { name: "PARAKEET_SIM_BF16_GEMM_OUT", owner: "npu-xrt", site: "npu-xrt/src/lib.rs",
        semantics: NotZero, default: "false",
    ...
    Flag { name: "PARAKEET_SIM_BF16_ONLY_N", owner: "npu-xrt", site: "npu-xrt/src/lib.rs",
    ...
    Flag { name: "NPU_DISPATCH_LOG", owner: "npu-xrt", site: "npu-xrt/src/lib.rs",
        semantics: NotZero, default: "false",
        doc: "logs per-(xclbin, insts) dispatch blocking time and hw_context-transition counts." },
    Flag { name: "NPU_XCLBIN_CACHE_BY_CONTENT", owner: "npu-xrt", site: "npu-xrt/src/lib.rs",
        semantics: NotZero, default: "false",
...
mod tests {
    ...
    fn every_flag_has_a_name_site_and_doc() {
        for f in FLAGS {
            ...
            assert!(f.site.ends_with(".rs"), "{}: site {:?} is not a rust file", f.name, f.site);
