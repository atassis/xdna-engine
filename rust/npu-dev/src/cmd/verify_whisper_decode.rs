...
pub fn run(argv: Vec<String>) {
    let host = argv.iter().any(|a| a == "--host");
    // --npu-attn = the NPU path with on-chip SELF-attention (fused LN+QKV + mha kernel) enabled.
    let npu_attn = argv.iter().any(|a| a == "--npu-attn");
    let npu = npu_attn || argv.iter().any(|a| a == "--npu");
    ...
    let redispatch = argv.iter().any(|a| a == "--redispatch");
    if !host && !npu {
        eprintln!(
