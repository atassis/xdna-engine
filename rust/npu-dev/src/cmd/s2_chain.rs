...
fn run_part(part: &str, chain: &S2DecoderChain, weights: &S2Weights, x: &Array2<f32>) -> Result<Vec<f32>, String> {
    match part {
...
fn main_inner(argv: Vec<String>) -> Result<(), String> {
    ...
    let out1 = run_part(part, &chain, &weights, &x)?;
    ...
    let out2 = run_part(part, &chain, &weights, &x)?;
