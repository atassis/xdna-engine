...
//! is RoPE-specific). fp32 `.npy` under artifacts/parakeet/encoder/{L0..L{N-1}, pre_encode, refs}.
...
impl ParakeetWeights {
    ...
    pub fn pre(&self, key: &str) -> &ArrayD<f32> {
        self.pre_encode.get(key).unwrap_or_else(|| panic!("missing pre_encode `{key}`"))
    }
