...
| GQA-aware decode attention (HD=128, M=1) | one streamed K/V tile, `TKV` keys x `HD=128` x 2 (K and V) | e.g. `TKV=64`: `[64,128]`x2 bf16 | -- | 32 KB | fits (`mha_decode`'s existing `TKV`-tile streaming pattern carries over directly; HD=128 doubles the per-tile byte count vs Whisper's HD=64, so `TKV` needs roughly halving to hold the same headroom -- a retune, not a redesign) |
