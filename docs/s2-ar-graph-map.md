...
|---|---|---|
| RMSNorm (attention_norm/ffn_norm/fast_norm, D=2560) | `aie_kernels/rmsnorm` | **REUSE**, D-generic `[tile,D]` contract already matches |
| RMSNorm on Q/K per head (D=128) | `aie_kernels/qk-norm` | **REUSE**, same contract, smaller D |
| SwiGLU (`silu(gate)*up`) | `aie_kernels/swiglu` | **REUSE**, exact match (`out=silu(gate)*up`, same as ggml's `ggml_vec_swiglu_f32`) |
