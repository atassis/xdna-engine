...
# ---- The FOURTH half: IRON, which no pin above can see ----
# The decode/prefill ELFs are GENERATED by IRON, and none of the gates above can read that.
# toolchain.lock pins mlir-aie and Peano; its IRON_FORK_COMMIT is a merge-base FLOOR for the
# checkout rather than a build identity; and a design's cache key enumerates the arguments that
# reach it, never the generator source. So an IRON change that silently alters emitted bytes
# leaves the artifact stamped identically to a correct one and every check above green.
#
# Paid for 2026-09-20 (IRON bc280c2, whose reason is in iron-artifact-floor): a coalesced GEMV's C
# tap was transposed against the core's emission order, so the gemma4-12b 256k arms answered
# correctly through position 2047 and returned digit garbage from 2048. Same toolchain hash, same
# arena pairing, same sequence_name, ELF size within 16 bytes -- meta.json's `iron.commit` was the
# only thing that differed between the two builds.
#
# WARN when the floor or an IRON tree is unreadable (a production root has no reason to carry a
# compiler checkout) but DIE on an artifact that RECORDS an IRON older than the floor: that one is
# not ambiguous.
IRON_FLOOR_FILE="$REPO/iron-artifact-floor"
iron_floor="$(sed -e 's/[[:space:]]*#.*$//' -e '/^[[:space:]]*$/d' "$IRON_FLOOR_FILE" 2>/dev/null | head -1 || true)"
# Resolve IRON_DIR through amd_paths.sh rather than a second copy of its default, in a subshell so
# that file's exports do not leak into the rest of this script.
iron_tree="$( . "$REPO/scripts/amd_paths.sh" >/dev/null 2>&1 && echo "${IRON:-${IRON_DIR:-}}" )" || iron_tree=""

if [ -z "$iron_floor" ]; then
  warn "no IRON artifact floor at $IRON_FLOOR_FILE -- staged artifacts are NOT gated on their generator."
elif [ -z "$iron_tree" ] || ! git -C "$iron_tree" cat-file -e "${iron_floor}^{commit}" 2>/dev/null; then
  warn "IRON floor $(printf %.12s "$iron_floor") not resolvable in ${iron_tree:-<no IRON tree>} -- artifacts NOT gated."
  iron_floor=""
else
  # A floor nobody can verify is the same silent failure with extra steps: the tree that would
  # REBUILD these artifacts has to contain the floor itself, or the gate below is unsatisfiable.
  git -C "$iron_tree" merge-base --is-ancestor "$iron_floor" HEAD 2>/dev/null \
    || die "the IRON tree that builds artifacts does not contain the artifact floor:
  $iron_tree at $(git -C "$iron_tree" rev-parse --short HEAD 2>/dev/null || echo '?')
  floor $(printf %.12s "$iron_floor") ($IRON_FLOOR_FILE)
  Merge the floor forward into that line, or lower the floor if it was set wrong."
fi

if [ -n "$iron_floor" ]; then
  iron_checked=0; iron_unstamped=0
  while IFS= read -r scen; do
    [ -n "$scen" ] || continue
    case "$scen" in /*) scen_abs="$scen" ;; *) scen_abs="$ENGINE_ROOT/$scen" ;; esac
    [ -f "$scen_abs" ] || continue
    # Both halves, unlike the pin loop above: a prefill ELF is generated by the same IRON and
    # shares decode's arena, so a defect in it is the same silent class.
    for key in decode prefill; do
      art=$(grep -oP "^\s*$key\s*=\s*\"\K[^\"]+" "$scen_abs" | head -1 || true)
      [ -n "$art" ] || continue
      case "$art" in /*) art_abs="$art" ;; *) art_abs="$ENGINE_ROOT/$art" ;; esac
      [ -f "$art_abs/meta.json" ] || continue
      ironc=$("$ONNX_ASR_PY" -c 'import json,sys
print((json.load(open(sys.argv[1])).get("iron") or {}).get("commit",""))' "$art_abs/meta.json" 2>/dev/null || true)
      if [ -z "$ironc" ]; then iron_unstamped=$((iron_unstamped + 1)); continue; fi
      if ! git -C "$iron_tree" cat-file -e "${ironc}^{commit}" 2>/dev/null; then
        warn "  $art: generated by IRON $(printf %.12s "$ironc"), absent from $iron_tree -- cannot gate it."
        continue
      fi
      git -C "$iron_tree" merge-base --is-ancestor "$iron_floor" "$ironc" 2>/dev/null || die "staged artifact predates the IRON artifact floor:
  $art_abs/meta.json
  generated by IRON $(printf %.12s "$ironc"), which does not contain $(printf %.12s "$iron_floor").
  $IRON_FLOOR_FILE records what that IRON gets wrong. Rebuild this artifact against the current
  IRON and re-run install:
    IRON=$iron_tree scripts/build_llm_decode.sh <spec> <layers> <out>
    IRON=$iron_tree DECODE_META=<out>/meta.json scripts/build_prefill.sh ..."
      iron_checked=$((iron_checked + 1))
    done
  done < <(grep -oP '^\s*scenario\s*=\s*"\K[^"]+' "$ENGINE_CONFIG")
  [ "$iron_unstamped" -eq 0 ] \
    || warn "$iron_unstamped staged artifact(s) record no IRON commit -- built before the field existed; rebuild to stamp them."
  ok "Staged artifacts pass the IRON floor ($iron_checked checked against $(printf %.12s "$iron_floor"))."
fi
