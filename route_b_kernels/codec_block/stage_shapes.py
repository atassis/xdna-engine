...
def head_plan():
    ...
    rd = NEW_RESIDENT_DEPTH
    total = conv_l1(HEAD_CI_CHUNK, k, resident_depth=rd, has_add=False)[-1]
    assert total <= L1_BUDGET, f"head @ ci_chunk={HEAD_CI_CHUNK}: {total} > {L1_BUDGET}"
    return dict(ci_chunk=HEAD_CI_CHUNK, resident_depth=rd)
