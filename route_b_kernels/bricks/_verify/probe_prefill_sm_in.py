...
M>=2 all NaN, so not row-count) the fault is inside the kernel body. RESOLVED 2026-08-31: prefill-attn is
device-green (32/32 heads, worst rel-L2 8.540e-08) once the aie.core stack reservation was
raised past its 0x400 default; this probe is kept as an instrument, not as current state. This finds WHICH stage.
    ...
    try:
        res = bricklib.verify_streamed(
            ...
            compile_flags=[f"-DPREFILL_HD={HD}", f"-DPREFILL_M={M}",
                           f"-DPREFILL_STOP_AFTER={stop}", f"-DPREFILL_VEC_EMIT={VEC}", f"-DPREFILL_SM_OUT_DIRECT={DIRECT}", f"-DPREFILL_SM_IN_STREAMED={INSTR}"],
        )
